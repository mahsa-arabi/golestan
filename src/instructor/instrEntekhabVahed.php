...
<head>
    ...
    <?php
    require_once('../student/stuStyle.php');
    if (isset($_POST['save'])) {
        $id = $_SESSION['ID'];
        $item = $_SESSION['item'];
        ...
        foreach ($item as $c_id) {
            ...
            $sql1 = "SELECT course_id FROM teaches WHERE ID='$id' AND course_id='$c_id' AND year='$year' AND semester='$semester'";
            $query1 = $conn->prepare($sql1);
            $query1->execute();
            $results = $query1->fetchAll(PDO::FETCH_OBJ);
            if ($query1->rowCount() === 0) {
                $sql = 'INSERT INTO `teaches` (`ID`, `course_id`, `sec_id`, `semester`, `year`) VALUES
                                                                                      (:id, :c_id, :s_id,:semester,:year);';
...
<div id="page">
  ...
  (SELECT course_id FROM section WHERE semester='$semester' AND year='$year' )";
                $query = $conn->query($sql);
                $results = $query->fetchAll(PDO::FETCH_ASSOC);
                if ($results) {
                    foreach ($results as $result) {
